...
def load_dino_encoder(encoder_name: str, weights_dir: Optional[str] = None):
    if "dummy" in encoder_name:
    ...
    if weights_dir is None:
        weights_dir = Path(__file__).resolve().parents[1] / "backbones" / "weights"
    ...
    if "dinov2" in encoder_name:
        return _load_dinov2_direct(encoder_name, weights_dir_path)

    raise ValueError(
        f"Unsupported encoder '{encoder_name}'. This submission package includes DINOv2 and dummy encoders only."
    )
...
def _load_dinov2_direct(encoder_name: str, weights_dir: Path):
    arch, patchsize = encoder_name.split("_")[-2], encoder_name.split("_")[-1]
    from .dino_backbones import vision_transformer as vision_transformer_dinov2
...
def build_inp_former(
    ...
    encoder = load_dino_encoder(encoder_name, weights_dir=weights_dir)
