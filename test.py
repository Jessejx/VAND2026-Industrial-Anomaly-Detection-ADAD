...
from utils.submission import save_anomaly_tiff, threshold_for
...
def test(args):
    ...
    output_dir = args.output_dir or test_cfg.get("output_dir", "outputs/vand2026_submit")
    ...
    threshold_cfg = test_cfg.get("thresholds", {})
    threshold_quantile = (
    ...
    use_config_thresholds = needs_binary_threshold and binary_threshold is None and bool(threshold_cfg)
    if use_config_thresholds:
        print("binary threshold: using per-category config thresholds")
    elif needs_binary_threshold and binary_threshold is None:
        ...
        for batch in tqdm(loader, desc="test", ncols=100):
            ...
            for i in range(maps.shape[0]):
                ...
                sample_threshold = (
                    ...
                    else threshold_for(item, img_type, threshold_cfg, default=0.5)
                )
                ...
                if save_tiff:
                    ...
                    written_tiff += 1
                if save_thresholded:
                    save_binary_thresholded_mask(anomaly_map, sample_threshold, item, img_type, name, output_dir)
                    written_masks += 1
...
def parse_args():
    ...
    parser.add_argument("--checkpoint", type=str, default="checkpoints/vand2026_inp_residual_noise.pth")
    ...
    parser.add_argument("--gpu_id", type=int, default=None, help="CUDA device index, for example --gpu_id 0")
