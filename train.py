...
def _log_synthetic_source(generator, enabled: bool) -> None:
    ...
    print(
        "[synthetic] enabled; anomaly_mode=random_noise "
        f"ratio={generator.anomaly_ratio} target_size={generator.target_size}"
    )
...
def build_dataset(cfg, data_root, categories):
    ...
    generator = SyntheticAnomalyGenerator(
        anomaly_mode=syn_cfg.get("anomaly_mode", "random_noise"),
        ...
        noise_blur_min=float(syn_cfg.get("noise_blur_min", 1.0)),
        noise_blur_max=float(syn_cfg.get("noise_blur_max", 4.0)),
    )
...
def train(args):
    ...
    save_dir = ensure_dir(train_cfg.get("save_dir", "checkpoints"))
    final_checkpoint_name = train_cfg.get("final_checkpoint_name", "vand2026_inp_residual_noise.pth")
    ...
    for epoch in range(start_epoch, epochs):
        ...
        if (epoch + 1) % save_interval == 0 or (epoch + 1) == epochs:
            ...
            if (epoch + 1) == epochs:
                final_path = Path(save_dir) / final_checkpoint_name
                save_checkpoint(str(final_path), model, optimizer, epoch, checkpoint_extra)
